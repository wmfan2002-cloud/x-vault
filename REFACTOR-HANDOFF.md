...
|---|---|---|---|---|
| style.css | 5569 | **58%** (80%) | 95 行 | <5% |
| admin.js | 2651 | **49%** (60%) | 167 行 | <5% |
| app.js | 2917 | **49%** (54%) | 245 行 | <5% |
...
- [x] 收藏按钮按产品要求调整：网格/紧凑视图位于卡片右上角，列表视图仍在操作栏，避免隐藏 Banner 后失去入口；收藏状态与事件契约保持不变。桌面/手机/紧凑三种已登录场景严格零差异（相对调整后的目标基线）。
