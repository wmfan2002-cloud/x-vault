...
2026-09-06 补充验收：`.visual/admin-original/`（`16d336b`）与 `.visual/admin-verified/` 的 12 个场景，
计算样式全部严格相同；11 个场景零像素差异，桌面列表角标字形边缘仍有 30 个差异像素，未宣称严格零差异。
旧工具曾将导出说明行高 `20.15px -> 20.8px` 误报为抖动，本轮已恢复原行高并收紧比较器。
原始 106 项测试之外，新增 6 组管理台交互测试并纳入 `npm test`。
...
进度（2026-09-06，工作流日志重构与回归网补强之后。行数为尺子使用的非空行数，括号里是起点）：
