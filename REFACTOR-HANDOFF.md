...
- [ ] `admin.js` 去派生：分析图表配置、两套排行榜骨架、博主下架乐观更新已抽成共享构造器；还剩 50% 块级重合、最长块 167 行 @1712，后续继续按面板职责推进。
...
5545 行，58% 落在连续相同块里，最长 95 行。按节推进，一节一验收。
- [x] 第 1-3 节 tokens / baseline / motion（约 300 行）：50 个设计 token 已换成语义命名，跨 `public/` 同步，22 个场景零差异。
- [ ] 第 4-5 节 header / hero
